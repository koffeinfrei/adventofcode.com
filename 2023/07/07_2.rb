...
CARDS_WITHOUT_JOKER = (CARDS - [JOKER])

def card_rank(char)
  CARDS.reverse.index(char) + 1
end
...
def type_with_joker_rank(hand)
  if hand.include?(JOKER)
    # remove the jokers, ...
    base_hand = hand.gsub(JOKER, '')
    # ... replace them with all possible other cards ...
    variants = (5 - base_hand.length).times.map { CARDS_WITHOUT_JOKER.zip.flatten }
    # ... and create all possible hand combinations
    permutations = [base_hand].product(*variants).map(&:join)
    permutations.map { type_rank(_1) }.max
  else
    type_rank(hand)
  end
end
...
sorted_hands = hands.sort do |a, b|
  ...
  a_type_rank = type_with_joker_rank(a)
  b_type_rank = type_with_joker_rank(b)
